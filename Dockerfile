...
RUN npm install mysql2@latest ejs body-parser express node-fetch dotenv bcryptjs express-session node-cron --save @sendgrid/mail
RUN npm install --save-dev proxyquire
RUN chmod +x /path/to/node_modules/.bin/mocha
COPY ./WebContent .
EXPOSE 3001
CMD ["npm", "start"]
